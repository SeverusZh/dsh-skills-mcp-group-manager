 ...
 *
 * UI: a left-docked management panel in the additive `shell.overlay` slot
 * (root scope) plus a toggle button in `conversation.session.header.actions`
 * (session scope). The panel manages skill groups and MCP servers by calling
 * the host RPC route `POST /plugins/dsh-mcp-skill-manager/rpc` (bundle
 * plugins have no `harness.handle`/`host.call`; body { method, args } →
