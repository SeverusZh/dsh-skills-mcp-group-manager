 ...
 *
 * Bundle plugin (installed via `dsh plugin --profile <p> add <path>`). Provides:
 *  1. State model & persistence — plugin-owned state file
 *     (<harness home>/mcp-skill-manager/state.json, atomic write; removed on
 *     uninstall via postuninstall). State: skill groups + MCP server entries.
 *  2. Skill-catalog filtering — per-agent shadow skill provider
 *     (`skill-manager-filter`) registered on `agent.ctx` at `agent/created`;
 *     skills outside the enabled-group union are rewritten to
 *     { modelInvocable: false, userInvocable: false }.
 *  3. MCP enumeration & toggling — profile servers disabled via per-agent
 *     `tools.restrict({ deny })`; user-added servers dynamically
 *     mounted/unmounted via `ctx.plugin` with the real dsh-mcp-client plugin.
 *  4. The 12 `manager_*` tools in the shared tools registry.
 *  5. RPC surface for the browser half: one POST route
 *     `/plugins/dsh-mcp-skill-manager/rpc` with body { method, args } →
 *     { ok, value } | { ok: false, error }.
 *
 * Lifecycle: every side effect is fiber-scoped or explicitly disposed. No
 * live data is ever serialized: only scalar fields are extracted into fresh JSON.
 */
...
export function apply(ctx, config = {}) {
  ...
  function applyAgentFilter(agent) {
    ...
    // agent.ctx.get('skills') — NOT agent.ctx.skills: the agent ctx's inject
    // map lacks 'skills', so property access throws the Cordis Guard error.
    ...
    const dispose = skills.registerProvider((control) => {
      ...
      // view to see agent-plane skills. Reading that view re-enters this
      // list() through the registry; the re-entrant call yields NO candidates
      // so the enclosing call can enumerate the raw catalog.
      ...
      return {
        ...
        async list(options) {
          ...
          try {
            ...
            // raw view (global + agent-plane providers, shadow stripped) —
            // exactly the catalog the agent sees, so the allowlist can
            // suppress agent-plane skills too.
            ...
            for (const [name, skill] of byName) {
              if (enabled.has(name)) {
                // Enabled agent-plane skills stay with their filesystem
                // provider: emitting them would shadow the real candidate's
                // get(), which cannot resolve agent-plane bodies.
        ...
        async get(candidate, options) {
          // Body loading delegates to the global view; disabled skills never
          // reach get() (the isModelInvocable gate refuses them after list()).
  ...
  // Reentrancy: tools.restrict() emits tools/change SYNCHRONOUSLY, and our
  // tools/change listener re-runs this function. Without the guard the
  // reentrant call sees no current restriction (the map entry is only written
  // after restrict() returns), recomputes the same deny set, and calls
  // restrict() again — unbounded synchronous recursion that stack-overflows
  // and wedges the whole dsh process. We also only swap when the deny set
  // actually changed (restrict → change → restrict → … would churn the
  // registry and flicker the UI).
  ...
  // Mounted as loader entries named '@deepseek-ai/dsh-mcp-client' (resolved
  // from the global dsh install, exactly like profile-configured rows) — the
  // host half never imports the mcp-client package, keeping `dsh plugin add`
  ...
  /** Parse { skill } | { skills: [...] } into a non-empty name list. */
  function parseSkillNames(args) {
    return Array.isArray(args?.skills)
  ...
  /** Batch group-membership mutation: ONE write + ONE invalidation for N names. */
  async function mutateGroupSkills(id, names, mutate) {
    return withWriteLock(async () => {
      const next = mutate(store.get(), id, names);
      if (next !== store.get()) {
  ...
  const api = {
    ...
    async skillsList() {
      ...
      // served by the agent-plane filesystem provider, so read each live
      // agent's view (global + agent layers) to list the full catalog.
      const byName = new Map();
    ...
    async groupsAddSkill(args) {
      const { id } = args ?? {};
      const names = parseSkillNames(args);
      if (typeof id !== 'string' || names.length === 0) {
        throw new Error('addSkill requires { id, skill } or { id, skills: [...] }');
      }
      // Echo the post-operation membership so callers (and the tool output
      // schema) get a confirmation payload, not an empty object.
      const skills = await mutateGroupSkills(id, names, addSkillsToGroup);
    ...
    async groupsRemoveSkill(args) {
      ...
      const skills = await mutateGroupSkills(id, names, removeSkillsFromGroup);
      return { id, skills: [...skills] };
    },
    async mcpList() {
  ...
  // `dsh plugin add` alone works (Node resolves the plugin's imports from the
  // linked source directory, which has no node_modules). The compact
  ...
  // Shared output-schema field fragments (identical JSON to the inline forms).
  const idField = { type: 'string', required: true };
  const nameField = { type: 'string', required: true };
  const enabledField = { type: 'boolean', required: true };
  const skillsField = { type: 'array', required: true, items: { type: 'string' } };
  const groupFields = { id: idField, name: nameField, enabled: enabledField, skills: skillsField };
  const idNameFields = { id: idField, name: nameField };
  const idEnabledFields = { id: idField, enabled: enabledField };
  const idSkillsFields = { id: idField, skills: skillsField };
  const serverNameField = { type: 'string', required: true };
  const serverNameEnabledFields = { serverName: serverNameField, enabled: enabledField };
  ...
  tool({
    ...
    output: {
      schema: {
        type: 'object',
        additionalProperties: false,
        properties: serverNameEnabledFields,
      },
  ...
  tool({
    ...
    output: {
      schema: {
        type: 'object',
        additionalProperties: false,
        properties: { serverName: serverNameField },
      },
