...
const RPC_PATH = '/plugins/dsh-mcp-skill-manager/rpc';
...
export function apply(ctx, config = {}) {
  ...
  async function mountMcpServer(server) {
    ...
    const fiber = ctx.plugin(
      { apply: dshMcpClient.apply, Config: dshMcpClient.Config, inject: dshMcpClient.inject, name: dshMcpClient.name },
      mcpClientConfig(server),
    );
    mountedMcp.set(server.serverName, fiber);
    // failOnStartupError: false keeps a failed connection from killing the
    // fiber; contain any setup rejection (e.g. duplicate serverName) so the
    // manager keeps working and the error is visible in logs.
    fiber.await().catch((error) => {
  ...
  async function unmountMcpServer(serverName) {
    const fiber = mountedMcp.get(serverName);
  ...
  const api = {
    ...
    async groupsAddSkill(args) {
      const { id, skill } = args ?? {};
      if (typeof id !== 'string' || typeof skill !== 'string' || skill.length === 0) {
        throw new Error('addSkill requires { id, skill }');
      }
      await withWriteLock(async () => {
        const state = store.get();
        ...
        if (!group.skills.includes(skill)) {
          await store.update({
            groups: state.groups.map((g) => (g.id === id ? { ...g, skills: [...g.skills, skill] } : g)),
          });
          applyStateEffects();
        }
      });
    ...
    async groupsRemoveSkill(args) {
      const { id, skill } = args ?? {};
      if (typeof id !== 'string' || typeof skill !== 'string') throw new Error('removeSkill requires { id, skill }');
      await withWriteLock(async () => {
        const state = store.get();
        requireGroup(state, id);
        await store.update({
          groups: state.groups.map((g) => (g.id === id ? { ...g, skills: g.skills.filter((s) => s !== skill) } : g)),
        });
        applyStateEffects();
      });
      return {};
  ...
  ctx.effect(() => () => {
    ...
    // mountedMcp fibers are children of this plugin fiber and are disposed by
    // Cordis itself; only drop the bookkeeping.
    mountedMcp.clear();
